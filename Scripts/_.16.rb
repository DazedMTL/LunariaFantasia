#==============================================================================
# ■ RGSS3 簡易アイテム合成ショップ Ver1.00　by 星潟
#------------------------------------------------------------------------------
...
class Window_ShopCommand < Window_HorzCommand
  ...
  def make_command_list
    ...
    name = $game_temp.shop_name.empty? ? EasyCompose::Name : $game_temp.shop_name
    a.each {|i| @list[i][:name] = name}
...
class Window_ShopStatus < Window_Base
  ...
  def refresh
    ...
    if @compose_mode_main && @item
        ...
        #contents.font = cfc
      #end
      draw_compose_list(0, line_height * 2)#4, line_height * 2)
    end
  end
  #--------------------------------------------------------------------------
  # 装備情報の描画
  ...
  def page_max
    (@compose_mode_main && (@item && (@item.is_a?(RPG::EquipItem) ? @compose_mode : false) ? @compose_mode : false)) ? 1 : page_max_easy_compose
  end
  #--------------------------------------------------------------------------
  # ページの更新
  #--------------------------------------------------------------------------
=begin
  ...
  def update_page
    ...
    if visible && Input.trigger?(EasyCompose::Key) && @item
...
=end
  ...
  def update_page
    if $game_switches[EasyCompose::SID]
      if window_change && @compose_mode && !@is_mode
        Sound.play_cursor
        @compose_mode = false
        refresh
      elsif window_change && !@compose_mode && @is_mode
        Sound.play_cursor
        @is_mode = false
        @compose_mode = true
        refresh
        ...
        @is_mode = true
        refresh
      elsif window_change_no_equip && !@compose_mode && !@is_mode
        Sound.play_cursor
        @compose_mode = true
        @is_mode = false
        refresh
      elsif window_change_no_equip
        Sound.play_cursor
        @compose_mode ^= true
        @is_mode ^= true
        refresh
      else
        if chara_change && Input.repeat?(:Y) && page_max > 1
          Sound.play_cursor
          @page_index = (@page_index + 1) % page_max
          refresh
        elsif chara_change && Input.repeat?(:X) && page_max > 1
          Sound.play_cursor
          @page_index = (@page_index - 1) % page_max
          refresh      
        end
  ...
  #--------------------------------------------------------------------------
  def chara_change
    visible && !@compose_mode && !@is_mode && (@item.is_a?(RPG::EquipItem) && !rune?(@item))
  end
  #--------------------------------------------------------------------------
  # 
  #--------------------------------------------------------------------------
  def window_change
    visible && Input.trigger?(EasyCompose::Key) && (@item.is_a?(RPG::EquipItem) && !rune?(@item))
  end
  #--------------------------------------------------------------------------
  # 
  #--------------------------------------------------------------------------
  def window_change_no_equip
    visible && Input.trigger?(EasyCompose::Key) && (@item.is_a?(RPG::Item) || rune?(@item))
  end
  #--------------------------------------------------------------------------
  # 合成素材リストの描写
  #--------------------------------------------------------------------------
  def draw_compose_list(x,y)
    ...
    draw_text(x,y,cw,lh,EasyCompose::Disc, 2)
    change_color(normal_color)
    i = 0
    @item.easy_compose_item_list.each do |k1,v1|
      v1.each do |k2,v2|
        case k1
        ...
        draw_text(x,yd,cw,lh,sprintf("%s/%s",n1,n2),2)#n2,n1),2)
        i += 1
      end
    end
    contents.font = cfc
